...
Based on Kolchyna's research on using social media and Google Trends data for sales forecasting.
...
SQL then selects the latest snapshot per `(date, keyword, product_id)` using `ROW_NUMBER()`.
...
From the Mention API, the pipeline uses `tone` (sentiment) and `author_influence_score` (author reach). Authors with larger reach have a greater impact on the weekly sentiment score. Weekly sentiment is an influence-weighted average, rescaled to 0–100 so it can be compared with Trends metrics:
...
The current mapping relies on exact keyword matches from `config/keywords.csv`. A more flexible approach would match mentions to products based on meaning rather than exact keywords.
...
In SQL (`sql/staging_views.sql`), `ROW_NUMBER()` keeps the most recent snapshot for each business key. Transformations (`sql/transformations.sql`) normalize trends, aggregate to weekly grain, and compute influence-weighted sentiment from mentions. Finally, `sql/load_fact_product_popularity.sql` loads trends and mentions into `fact_product_popularity`.
...
- `sql/staging_views.sql` – snapshot deduplication views
